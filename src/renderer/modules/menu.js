...
const MENUS = {
  ...
  help: [['shortcuts', '快捷键', 'Keyboard shortcuts', ''], ['about', '关于 LMark', 'About LMark', '']]
};
...
export function createMenuController({ onAction }) {
  ...
  function open(name, trigger) {
    ...
    MENUS[name].forEach(([id, zh, en, shortcut]) => { const label = getLocale() === 'en' ? en : zh; const item = document.createElement('button'); item.className = 'menu-item'; item.setAttribute('role', 'menuitem'); item.innerHTML = `<span>${label}</span><span class="menu-shortcut">${shortcut}</span>`; item.onclick = () => { onAction(id); close(); }; popover.append(item); });
